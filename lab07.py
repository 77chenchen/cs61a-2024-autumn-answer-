...
def duplicate_link(s, val):
    ...
    while s is not Link.empty:
        if s.first == val:
            s.rest = Link(val, s.rest)  # Insert duplicate immediately after
            s = s.rest.rest  # Skip over the newly inserted duplicate
        else:
            s = s.rest  # Move to the next element
