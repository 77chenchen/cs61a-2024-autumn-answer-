...
def balanced(m):
    ...
    if is_planet(m):
        ...
        left_torqe=length(left(m))*total_mass(end(left(m)))
        right_torge=length(right(m))*total_mass(end(right (m)))
        return (left_torqe==right_torge and balanced(end(left(m))) and balanced(eｎｄ(right(m))))
