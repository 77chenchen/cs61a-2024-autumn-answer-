...
def boar_brawl(player_score, opponent_score):
    ...
    if o_s < 10:
        ...
        t_d=opponent_score//10%10
    ones_digits=player_score%10
    if ones_digits >= t_d:
        scores=(ones_digits - t_d)*3
        if scores > 1:
            return scores 
        else :
            return int(1)
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    b=player_score
    if num_rolls == 0:
        scores=boar_brawl(b,a)
    else:
        scores=roll_dice(c_num,current_dice)
    return scores
...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    cc=opponent_score
    dd=dice
    player_score+=take_turn(aa,bb,cc,dd)
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score in range(goal+1):
        for oppoent_score in range (goal+1):
            if strategy(score,oppoent_score) != common:
                return False
    return True
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    for num_rolls in range (1,11):
        a_v=make_averaged(roll_dice,times_called)(num_rolls,dice)
        a_v_list[num_rolls]=a_v
    min_num_rolls=max(a_v_list,key=a_v_list.get)
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    now_socres=sus_update(0,score,opponent_score,dice=six_sided)
    if (now_socres-score)>=threshold:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
